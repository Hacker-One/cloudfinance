...
const OrderNotFound = "404"


func getAccountidByUser(orderurl string,user string) (string, error) {
	resp, err := http.Get(fmt.Sprintf("%s/users/%s/accounts",orderurl, user))
	if err != nil {
		return "", fmt.Errorf("could not get accountid %s: %s", user, err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("could not unmarshal scope info %q: %s", string(body), err)
	}
        acctid := res.Get("data").Get("id").MustString()
	return acctid, nil
}

func validateScope(orderurl string,accountid string, rtype string, rpath string) (string, error) {
	resp, err := http.Get(fmt.Sprintf("%s/artifacts?accountid=%s&type=%s&path=%s",orderurl, accountid, rtype, rpath))
	if err != nil {
		return "ERR", fmt.Errorf("could not get scope %s: %s", accountid, err)
	}
	defer resp.Body.Close()
	body, _ := ioutil.ReadAll(resp.Body)
        ...
        res, err := simplejson.NewJson([]byte(body))
	if err != nil {
		return "ERR", fmt.Errorf("could not unmarshal scope info %q: %s", string(body), err)
	}

        rcode := res.Get("code").MustString()
	...
	for _, a := range ares {
		...
		if ra.Actions == nil {
			ra.Actions = []string{}
                       ...
                       accid,err := getAccountidByUser(as.config.Server.OrderUrl,  ar.User)
	               if err != nil {
                          glog.Warningf("Get AcctountId Error %s", err)
                       }

                       rtype := "docker"
                       rcode,err := validateScope( as.config.Server.OrderUrl, accid , rtype, ra.Name ) 
	               if err != nil {
                          glog.Warningf("Get scope Error %s", err)
                       }

                       if  rcode == OrderNotFound {
			ra.Actions = []string{}
                       }
	               glog.Infof("zk Check scope:[%s][%s][%s][%s][%s][%s][%s]", as.config.Server.OrderUrl, ar.User, accid , rtype, ra.Name ,rcode,ra.Actions)
                }
